...
Version 0.2
Reads in OTS4 .txt files and extracts strain, frequency, and amplitude data from all files.
...
# specift the row containing each dataset extracted from the files
header = 14 # rows
p_row = 16 - 1 - header  # Adjusting for 0-based index in Python
s_row = 21 - 1 - header
f_row = 22 - 1 - header
a_row = 23 - 1 - header
time_row = 19 - 1 - header

# Specify the folder where the DSS .txt files are
...
while input("Do you want to continue? Type 'y' to continue.\n") == "y":
    ...
    # Loops over the list of .txt files
    ...
    for k in range(num_files): # uncomment this to process all files in the folder.
        ...
        if k == 0:
            ...
            strain = data[s_row].replace('|', ' ').split()[2:]  # Skips the first 2 entries 'Temperature' and 'profile'
            strain = np.array(strain, dtype=float)
    
            frequency = data[f_row].replace('|', ' ').split()[784:]  # Skips the first 3 entries 'Peak' 'frequency' 'profile' and internal measurements
            frequency = np.array(frequency, dtype=float)
            
            amplitude = data[f_row].replace('|', ' ').split()[784:]  # Skips the first 2 entries 'Amplitude' and 'profile'
            amplitude = np.array(amplitude, dtype=float)
    
            time_hist.append(data[time_row][1:21])
    
            s_values.append(strain)
            f_values.append(frequency)
            a_values.append(amplitude)
            
        else:
            # Append temperature data for subsequent files
            strain = data[s_row].replace('|', ' ').split()[2:]
            strain = np.array(strain, dtype=float)
...
S = np.column_stack((dt, s_values))
F = np.column_stack((dt, f_values))
A = np.column_stack((dt, a_values))
    
# Create a DataFrame for easy manipulation
df_P = pd.DataFrame(position)
df_S = pd.DataFrame(S, columns=["ElapsedTime"] + [f"Strain_{i}" for i in range(1, S.shape[1])])
...
df_P_path = os.path.join(directory, "PIRO_DSS_locations.pkl")
df_S_path = os.path.join(directory, "PIRO_DSS.pkl")
df_F_path = os.path.join(directory, "PIRO_DSS_Freq.pkl")
df_A_path = os.path.join(directory, "PIRO_DSS_Amp.pkl")

df_P.to_pickle(df_P_path)
df_S.to_pickle(df_S_path)
